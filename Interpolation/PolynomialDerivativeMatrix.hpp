 ...
 *  Output (implicit), matrix<double> derivative_matrix(N,N) : differentiation
 *    matrix
 *
 */
{
  size_t n_nodes{
      derivative_matrix.size2()};  // number of columns in derivative_matrix

  assert((n_nodes == derivative_matrix.size1()) &&
         "polynomial_derivative_matrix - matrix is not square");
  assert((n_nodes == nodes.size()) &&
         "polynomial_derivative_matrix - nrows != vector dimension");
