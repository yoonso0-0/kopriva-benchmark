...
void polynomial_and_derivative(const int poly_degree_N, const double x_eval,
                               double& L_x, double& dL_x) {
  /*
...
void q_and_L(const int poly_degree_N, const double x_eval, double& q_x,
             double& dq_x, double& L_x) {
  /*
   *  Evaluates q, q', L_N(x)
  ...
  if (poly_degree_N == 0) {
    ...
    L_x = L_k;
    q_x = (L_k * x_eval - L_k_minus_2) * (2 * poly_degree_N + 1) /
          (poly_degree_N + 1);
